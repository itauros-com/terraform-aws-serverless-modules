...
| <a name="input_role_name"></a> [role\_name](#input\_role\_name) | Name of the role EventBridge Scheduler assumes. Derived from the schedule's name when<br/>null, which is what you want unless a role already exists under another name.<br/><br/>The derived name is shortened when `<prefix>-<name>-scheduler` runs past the 64<br/>characters IAM allows, keeping 55 characters and appending 8 hex of the full name's<br/>hash. Set this explicitly to choose the short name yourself. | `string` | `null` | no |
